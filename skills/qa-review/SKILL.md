---
name: qa-review
description: 구현 결과를 명세 기준으로 검증하고 테스트 케이스, 검증 결과, 결함 및 릴리즈 판단 메모를 정리하는 QA 스킬
---
...
### 선택 입력
- DOMAIN_MODEL / VALIDATION_RULES / ERROR_POLICY
- 과거 버그 메모

## 출력
- output/qa/TEST_CASES.md
- output/qa/QA_SUMMARY.md
- 필요 시 결함 목록 또는 회귀 위험 메모
...
5. 릴리즈 판단과 후속 액션을 정리한다.

## 품질 기준
- 테스트 케이스만이 아니라 결과 요약이 있다.
- blocker 와 미검증이 구분된다.
- 재현 가능하거나 검증 가능한 문장으로 적는다.
- builder 가 바로 수정할 수 있는 수준의 정보가 있다.

## Self-Review
- 기능별이 아니라 시나리오별로 봤는가
- 예외 상황이 포함됐는가
- 실행 못 한 테스트를 숨기지 않았는가
- 릴리즈 판단에 근거가 있는가
...
## 다음 단계 handoff
- 실패 시: `builder-implementation`
- 통과 시: 사용자 또는 릴리즈 판단 단계
- handoff packet:
  - blocker
  - medium risk
  - 미검증 항목
...
### TEST_CASES 예시
1. 고객명 없이 저장 시 오류 메시지가 보인다.
2. 작업종류 수정 후 저장하면 상세 화면에 반영된다.
```
